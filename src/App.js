...
const App = () => {
  ...
  const userPreferences = {
    vacationStylePreferences: vacationStyleItems.filter(
      (item) => item.category === 1
    ),
    venturesPreferences: venturesItems.filter((item) => item.category === 1),
    destinationsPreferences: destinationsItems.filter(
      (item) => item.category === 1
    ),
  };
  ...
  return (
    <div>
      ...
      {currentPage === "vacation-style" && (
        <VacationStyle
          items={vacationStyleItems}
          setItems={setVacationStyleItems}
          VacationStyle
          onNextClick={ventureClick}
          onHomeClick={handleHomeClick}
          addVacationStyle={addVacationStyle}
        />
      )}
      {currentPage === "ventures" && (
        <Ventures
          items={venturesItems}
          setItems={setVenturesItems}
          onBackClick={styleClick}
          onHomeClick={handleHomeClick}
          onNextClick={destClick}
          addVacationVenture={addVacationVenture}
        />
      )}
      {currentPage === "destinations" && (
        <Destinations
          items={destinationsItems}
          setItems={setDestinationsItems}
          onBackClick={ventureClick}
          onHomeClick={handleHomeClick}
          onNextClick={itineraryClick}
          addVacationDestination={addVacationDestination}
        />
      )}
      ...
      {currentPage === "itineraries" && (
        <ItineraryList
          ...
          setGeneratedLocations={setGeneratedLocations}
        />
      )}
      {currentPage === "firstItineraries" && (
        <GPT
          onBackClick={itineraryClick}
          userPreferences={userPreferences}
          onLocationsUpdate={handleLocationsUpdate}
          locations={locations}
